...
<table class="nui-table-inner">
	<thead class="nui-table-head">
    	...
    	if(!rsf.get("infectant_name").equals("流量2")){
   			 %>
			<th class="nui-table-cell  " title="<%=rsf.get("infectant_unit")%>">
  ...
  %>
	    ...
	    <td width=7%><%=f.sub(rs.get("m_time"),10,9)%></td>
	        <%for(int i=0;i<infectant_list.size();i++){ %>
		        <td width=<%=56/size%>% class='<%=infectant_css.get(i) %>'><%=f.format(infectant_list.get(i),"0.#####")%></td>
		    <%
...
<script>
 function f_real_view(station_id,area_id){
 	window.parent.document.form3.station_id.value=station_id;
 	window.parent.document.form3.area_id.value=area_id;	
	window.parent.document.form3.action="../station_new/index_one.jsp";
	window.parent.document.form3.submit();
...
function f_view_comment(station_id){
	window.parent.document.form3.station_id=station_id;
    ...
    window.parent.get(zc.value,yj.value,bj.value,tj.value);
