                   ...
                   --%>
								<div style='padding-top:8px'>
									<select name='data_table' id='data_table'
										onchange='f_btn();f_submit()' class='search'
										style="width:120px">
                       ...
                       --%>
									<input type='text' class='input' style="height:20px"
										name='date1' id='date1' value='<%=w.get("date1")%>'
										onclick="new Calendar().show(this);"> 
									<select name=hour1 id='hour1' class='search' style="width:50px">
										<%=w.get("hour1Option")%>
									</select> 
									<input type='text' class='input' style="height:20px"
										name='date2' id='date2' value='<%=w.get("date2")%>'
										onclick="new Calendar().show(this);"> 
                       ...
                       --%>
									...
									<input type='button' value='完好率统计' class='btn' onclick='f_wh()'	id='btn_whl'> 
					...
					<div class='scrolldiv'>
						<table style="cursor:hand" border=0 cellspacing=0
							...
							<%
										...
										%>
							<tr>
								<td style="padding-top:4px;" id='td<%=index%>'
									title="<%=station.get("station_desc")%>"
									onclick=f_station_click(<%=index%>,'<%=station.get("station_id")%>','<%=station.get("station_desc") %>','<%=station.get("station_type")%>') >
									<font id='station_desc_<%=index%>'
									style="width:<%=td_width-10%>px;font-size:13px;overflow:hidden;white-space:nowrap;"><%=station.get("station_desc")%></font>
									<input type="hidden" value="" id="station_desc_title" name="station_desc_title"></td>
							</tr>
							<%}%>
						</table>
					</div></td>
...
<%if(!f.empty(station_id_0)){%>
  f_station_click(0,'<%=station_id_0%>','<%=station_desc_0%>',<%=station_type%>);
<%}%>

  function f_station_click(index,station_id,station_name,station_type){
...
//    form1.station_type.value=station_type;
  form1.action='./data/his.jsp?station_type='+station_type;
  form1.submit();
  //f_set_css(0);
 ...
 function f_btn_real(){
      ...
      f_ajf_show("date1,date2,hour1,hour2,btn_bulu,btn_whl");
 }
 function f_btn_avg(){
   // f_hide_all();
  // f_ajf_hide("date3,hour3,hour1,hour2");
  	f_ajf_hide("hour1,hour2,btn_whl,btn_bulu");
 ...
 function f_btn(){
	...
	var data_table = form1.data_table.value;
 	var url = "./data/bulunew.jsp";
	url = url+"?station_id="+station_id+"&station_desc_title="+escape(escape(document.getElementById("station_desc_title").value))+"&date1="+date1+"&date2="+date2+"&hour1="+hour1+"&hour2="+hour2+"&data_table="+data_table+"&station_type="+station_type;
	var width = 1024;
	var height = 568;
	...
	url = url+"?station_id="+station_id+"&station_desc="+escape(escape(document.getElementById("station_desc").value));
	var width = 812;
	var height = 356;
	window.open(url,"","scrollbars=yes,resizable=yes,height=356,width=812,left="+(window.screen.width-width)/2+",top="+(window.screen.height-height)/2);
	 ...
	 function f_wh(){
		...
		var hour2 = form1.hour2.value;
		var station_type = <%=station_type%>;
	 	var url = "./search/p_wh.jsp"+"?station_id="+station_id+"&date1="+date1+"&date2="+date2+"&hour1="+hour1+"&hour2="+hour2+"&station_type="+station_type;
