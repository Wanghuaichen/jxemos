...
public class NewChart_bak {
	...
	public static String getRealSql(String station_id,
		...
		if(f.empty(date3)){date3=f.today();}
		h = f.getInt(hour3,0);
		if(h>23){h=23;}
		...
		Object[] p=new Object[]{station_id,infectant_id,date3,date3,hour3};
		sql=f.str(sql,p);
		...
		try {
			...
			for (i = 0; i < num; i++) {
				...
				if(lcmap.get("max")!=null&&lcmap.get("min")==null){
					...
					if(yValue>StringUtil.getDouble(lcmap.get("min") + "", 0)&&yValue<StringUtil.getDouble(lcmap.get("max") + "", 0)){
						arry[i] = yValue;
						arrx[i] = (java.util.Date) xList.get(i);
					}
				}else{
	 ...
	 */
	public static Map getLch(String infectant_id,HttpServletRequest req)throws Exception{
		Connection cn = null;
		Statement stmt = null;
		ResultSet rs = null;
		Map map = new HashMap();
		try {
			cn = DBUtil.getConn(req);
			stmt = cn.createStatement();
			String sql = "select t.lo_min,t.hi_max from t_cfg_infectant_base t where t.infectant_id='"+infectant_id+"'";
			rs = stmt.executeQuery(sql);
			while(rs.next()){
	...
	/*!
	 *  根据数据集合sqlList，图表宽度w，图表高度h，顶部高度top,底部高度bottom,左边距left,右边距right和request值，查询数据库，获得图表信息
	 */
	...
	/*!
	 *  根据最大行数num，图表宽度w，图表高度h，顶部高度top,底部高度bottom,左边距left,右边距right和request值，查询数据库，获得图表信息
	 */
	...
	public static String getChartWithLimit(String sql, int w, int h, int top,
		...
		if(StringUtil.equals(bar,"1")){
			...
			if (hihi > 0) {
				//c.addInterLineLayer(null,c.yAxis().addMark(hihi, 0xff0000,hihi+"").getLine(),-1);//黄宝修改
				c.addInterLineLayer(null,c.yAxis().addMark(hihi, 0xff0000,((int)hihi)+"").getLine(),-1);
			}
			if(hi>0){
				//c.addInterLineLayer(null,c.yAxis().addMark(hihi, 0xff0000,hi+"").getLine(),-1);//黄宝修改
				c.addInterLineLayer(null,c.yAxis().addMark(hi, 0xff0000,((int)hi)+"").getLine(),-1);
			}
		...
		if (hihi > 0) {
			...
			//c.addAreaLayer(arry, c.yZoneColor(hihi, -1, 0xffff00));
			//报警线 20070926
			//c.addInterLineLayer(lineLayer.getLine(0),c.yAxis().addMark(hihi, 0xff0000,hihi+"").getLine(),-1);//黄宝修改
			c.addInterLineLayer(lineLayer.getLine(0),c.yAxis().addMark(hihi, 0xff0000,((int)hihi)+"").getLine(),-1);
		}
		...
		if (hi > 0) {
			...
			//预警线  20070926
		//c.addInterLineLayer(lineLayer.getLine(0),c.yAxis().addMark(hi, 0xffff00,hi+"").getLine(),-1);//黄宝修改
			c.addInterLineLayer(lineLayer.getLine(0),c.yAxis().addMark(hi, 0xffff00,((int)hi)+"").getLine(),-1);
	...
	public static String getChartWithLimit(int maxRows,String sql, int w, int h, int top,
		...
		if (hi > 0) {
			...
			a1.setXData(arrx);
			//c.addAreaLayer(arry, c.yZoneColor(hi, -1, 0xff0000));
			c.addInterLineLayer(lineLayer.getLine(0),c.yAxis().addMark(hi, 0xffff00,hi+"").getLine(),-1);
		}
...
//		}
	...
	public static String getRealChart_new(int num, int w, int h, int top,
			int bottom, int left, int right,String title, HttpServletRequest req,String chart_form)
			throws Exception {
		String url = null;
		String station_id = req.getParameter("station_id");
		String infectant_id = req.getParameter("infectant_id");
		...
		if(chart_form.equals("1")){
			...
			url = chart.getChart(sql, w, h, top, bottom, left, right,title,fmt,req);
		}
		return url;
	}
	/*!
