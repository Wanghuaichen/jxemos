...
<%
	...
	try {
		...
		try {
			hour1 = Integer.parseInt(request.getParameter("hour1"));
		} catch (Exception e) {
			hour1 = StringUtil.getNowHour();
		}
		try {
			hour2 = Integer.parseInt(request.getParameter("hour2"));
		} catch (Exception e) {
			hour2 = StringUtil.getNowHour();
		}
		...
		infectantCol = App
				.getInfectantCol(cn, station_id, infectant_id);
		...
		if (view_flag.equals("false")) {
			sql = "select  m_time,"
					+ infectantCol
					+ " as m_value,val01,val02,val03,val04,val05  from "
					+ data_table + " ";
			...
			double[] arrDoubleVal = NewChart.getLimitValue(station_id,
					infectant_id, request);
			String url = "";
			String fileName = "";
			...
			sql = "select m_time" + s + " from " + data_table
					+ " where station_id='" + station_id
					+ "' and m_time>='" + date1 + "' and m_time<='"
					+ date2 + "' order by m_time asc";
			list = chart.getChartImg(sql, "yy-mm", chartTitle, request,
					station_id);
