...
<%
 ...
 throw new Exception("请选择用户");
  ...
  userStationMap = RealDataQuery.getStationQuery(request,user_id); 
...
<input type=hidden name=cols value="user_id,station_ids" >
...
<%if(mod==1){out.println("<tr>");}%> 
   <td>
   <input type=checkbox name=chb_station_id value="<%=rs.get("station_id")%>" <%=flag%>><%=rs.get("station_desc")%>
   </td>
<%
...
function u(){
var item=document.all["chb_station_id"]; 
var str = "";
