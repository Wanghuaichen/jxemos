...
public class WarningAnyChart {
	...
	public static String getWarningData(HttpServletRequest request) {
		// String start_time = StringUtil.getNowDate() +"";//当天数据
		String start_time = "2009-4-13";// 测试数据日期
		...
		xmlString += "<anychart><settings><animation enabled='True' /></settings><charts><chart plot_type='CategorizedVertical'>"
				...
				+ "站位编号: {%SeriesName}{thousandsSeparator:,numDecimals:0} \n"
				+ "次数: {%Name}{numDecimals:0} \n"
				+ "数值: {%YValue} \n"
				+ "污染因子: {%inid}{numDecimals:0}  \n"
				+ "报警类型: {%wtp}{numDecimals:0}"
		...
		xmlString += "</data>"
				...
				+ start_time
				+ " 报警数据</text><background enabled='false' /></title><chart_background enabled='false'/> "
				+ "<axes> <y_axis><title> <text>平均值</text></title></y_axis> "
				+ "<x_axis tickmarks_placement='Center'><labels /> <title><text>次数</text> "
	...
	public static String getWarningOfMonth(HttpServletRequest req, String date) {
		if (StringUtil.isempty(date)) {
			date = (StringUtil.getNowDate() + "").substring(0, 7);// 当前时间
		}
		...
		String xmlString = "<?xml version='1.0' encoding='UTF-8'?>";
		xmlString += "<anychart><settings><animation enabled='True' /></settings><charts><chart plot_type='CategorizedVertical'>"
				+ "<chart_settings><title><text>报警次数（按天计）</text><background enabled='false' /></title><chart_background enabled='false'/> "
				+ "<axes><y_axis><title> <text>次数"
				...
				+ "<line_series><tooltip_settings enabled='true'><format>"
				+ "日期:{%inid} \n"
				+ "报警次数: {%YValue}{numDecimals:0}"
				+ "</format></tooltip_settings> </line_series></data_plot_settings><data><series name='报警数据'>";
		for (int i = 1; i <= 31; i++) {
