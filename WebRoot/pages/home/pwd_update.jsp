...
<%
         ...
         cn = DBUtil.getConn();
          ...
          sql = "select pwd_lock from t_sys_user where user_id='"+user_id+"'";
          m = DBUtil.queryOne(cn,sql,null);
          if(m!=null){pwd_lock=(String)m.get("pwd_lock");}
           if(StringUtil.equals(pwd_lock,"1")){
               throw new Exception("该用户密码已锁定，不能修改");
           }

         //if(2>1){out.println(user_id+","+pwd_lock+","+sql);}
         sql = "select user_id from t_sys_user where user_id=? and user_pwd=?";
         ...
         //System.out.println("pwd===="+pwd_old);
         m = DBUtil.queryOne(cn,sql,new Object[]{user_id,pwd_old});
         if(m==null){
         throw new Exception("原密码不正确");
         }
         
         sql = "update t_sys_user set user_pwd=? where user_id=? ";
